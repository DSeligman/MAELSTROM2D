...
int main()
{
  ...
  t = clock();
  string line;
  ifstream myfile ("params_SkewShearDblK.txt");
  double * inputs=NULL;
  ...
  for(int i=0;i<nx;i++){
    for(int j=0;j<ny;j++){
      ...
      ux[i*ny+j]=.1*cos(4.*(-1.*xg[i]+yg[j]))/sqrt(2.);
      uy[i*ny+j]=.1*cos(4.*(-1.*xg[i]+yg[j]))/sqrt(2.);
  ...
  ofstream outputFile("output_SkewShearDblK/rho.txt"); // output filestream                           
  ofstream outputFile2("output_SkewShearDblK/ux.txt");    //read out the initial velocity flow           
  ofstream outputFile3("output_SkewShearDblK/uy.txt"); ////////                                          
  ofstream outputFile4("output_SkewShearDblK/E.txt");
